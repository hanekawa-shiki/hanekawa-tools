...
> 最后更新：2026-07-21
> 最新 commit：`df50e0c9` (feat: add pwa)
...
│   │   └── avatar.jpeg            # 侧边栏头像图片
│   ├── components/
│   │   ├── app-sidebar.tsx        # 侧边栏：头像（SidebarFooter）+ 导航菜单（useMemo 缓存）
│   │   ├── nav-main.tsx           # 导航菜单（Base UI render 语法 + hugeicons）
│   │   ├── mode-toggle.tsx        # 深色/浅色/跟随系统切换
│   │   ├── page-header.tsx        # 页面标题组件（IntersectionObserver 滚动阴影）
│   │   ├── sw-update-toast.tsx    # PWA 版本更新提示组件（sonner toast）
│   │   ├── theme-provider.tsx     # 主题上下文 Provider（自定义，非 next-themes）
...
│   │   ├── index.tsx              # 路由入口（Lazy + Suspense + Layout 组装）
│   │   └── not-found.tsx          # 404 路由组件
│   └── types/                     # 全局类型声明（.d.ts，无需 import）
│       ├── globals.d.ts           # 主入口（引用其他 .d.ts）
│       ├── api.d.ts               # API 相关类型（ApiRequestConfig、HolidayApiResponse 等）
│       ├── holidays.d.ts          # 节假日类型（Holiday 等）
│       ├── pwa-register.d.ts      # PWA virtual:pwa-register/react 类型声明
...
- **所有自动路由和手动路由均包裹 `<Suspense>`**，支持代码分割加载状态
...
- Service Worker 注册在 `src/components/sw-update-toast.tsx` 中通过 `useRegisterSW` Hook 处理
- 更新检测时机：页面刷新或路由切换时浏览器自动检查 SW 是否有新版本，新资源下载完成后触发提示
- 更新提示：通过 sonner toast 在右下角显示"发现新版本"，用户可选择"刷新"或"稍后"
- 类型声明：`src/types/pwa-register.d.ts` 提供 `virtual:pwa-register/react` 的 TS 类型
- 离线缓存策略：
...
Vite `rollupOptions.output.manualChunks` 按功能将第三方库拆分为独立 chunk：

| Chunk 名         | 包含的库                                     |
| ---------------- | -------------------------------------------- |
| `vendor-react`   | react, react-dom, react-router               |
| `vendor-pdf`     | pdf-lib                                      |
| `vendor-dnd`     | @dnd-kit/react, @dnd-kit/dom                 |
...
所有页面组件通过 `React.lazy` + `<Suspense>` 按路由懒加载。
...
| 路由懒加载      | ✅ 完成 | `src/router/index.tsx`（React.lazy + Suspense 包裹所有路由）                         |
| 侧边栏导航      | ✅ 完成 | `src/components/nav-main.tsx`, `app-sidebar.tsx`（useMemo 缓存菜单）                 |
...
| PWA 更新提示    | ✅ 完成 | `src/components/sw-update-toast.tsx`（页面刷新/路由切换时检测，sonner toast 右下角） |
| 代码分割        | ✅ 完成 | `vite.config.ts` manualChunks + React.lazy 路由懒加载                                |
...
>
> **技术栈**：React 19 + Vite 8 + TypeScript 6 + Tailwind CSS 4 + Base UI (shadcn base-maia) + Hugeicons + react-router 8 + axios + dayjs + lunisolar + sonner + vite-plugin-pwa + workbox-window
>
> **关键约定**：
>
...
> - 所有页面组件通过 React.lazy + Suspense 按路由懒加载，Vite manualChunks 拆分第三方库
> - 主题使用项目自身的 ThemeProvider（非 next-themes）
>
> 项目当前功能包含：自动路由系统（含懒加载 + Suspense）、侧边栏导航（useMemo 缓存）、头像区域、主题切换、日历万年历（含节假日 API 接口）、油价查询（全国各地最新油价）、种子转磁力链（含逐条删除 + 清除全部）、发票合并（@dnd-kit 拖拽排序 + pdf-lib A4 PDF 合并导出）、字体切换插件（dev 本地包 / prod CDN）、PWA 可安装应用（prompt 更新提示 + Workbox 离线缓存）、404 页面、按类型着色的全局 Toast 通知。
>
